var TickSnd = new buzz.sound('snd/tick', {
  formats: ['mp3', 'ogg', 'wav']
});
TickSnd.load();
...
(new Image()).src = 'img/toggle_btn_depressed.jpg';

/**
 * MetronomeApp module.
 */
var MetronomeApp = angular.module(
  'MetronomeApp', [], function() {});
...
/**
 * bpmInput directive helps with responsive input font size.
 */
MetronomeApp.directive('bpmInput', function($window) {
  return function(scope, elem) {
    var $jqWindow = $($window);
    $jqWindow.resize(function() {
      elem.css('font-size', elem.height() * 0.8 + 'px');
    });
...
var MainCtrl = function($scope, $timeout) {
  ...
  $scope.$watch('bpm', function(bpm) {
    ...
    if(!self.bpmInRange()) {
      ...
      var validator = $scope.bpm < $scope.minBpm
        ? 'min' : 'max';
...
MainCtrl.prototype.toggleMetronome = function() {
...
MainCtrl.prototype.beat = function() {
  var self = this;
...
MainCtrl.prototype.getWandSwingTransitionDuration
= function() {
  ...
  return this.$scope.swingToCenter && !this.bpmInRange()
    ? '1.5s'
    : this.getBeatMs() / 1000 + 's';
};

MainCtrl.prototype.getBeatMs = function() {
...
MainCtrl.prototype.bpmInRange = function() {
  return this.$scope.bpm >= this.$scope.minBpm
      && this.$scope.bpm <= this.$scope.maxBpm;
};

MainCtrl.prototype.getInputErrorText = function() {
  var errorText;
